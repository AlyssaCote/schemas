from version2messagehandler import *
...
# INDIRECT
indirect_mh = IndirectMessageHandler()
...
# DIRECT TORCH
direct_mh = DirectMessageHandler()
...
t1 = direct_mh.create_tensor(torch1, str(torch1.layout), "int8")
# t1_descriptor = direct_mh.create_tensor_descriptor(list(torch1.shape), str(torch1.layout), 'int8')
t2 = direct_mh.create_tensor(torch2, str(torch2.layout), "int8")
# t2_descriptor = direct_mh.create_tensor_descriptor(list(torch2.shape), str(torch2.layout), 'int8')

t_list = [t1, t2]

# version with model data
direct_request_data = direct_mh.create_request(
    "torch_direct_model_name",
    t_list,
    "reply channel byte string",
    "cpu",
    "torch",
...
# DIRECT TF
tf_direct_mh = DirectMessageHandler()
